...
* Lisp はガベージコレクタにより変数の解放の必要がない。クロージャによる情報の保持も可能
* メモ化は、関数が受け取った引数とその結果を毎回記録し関数呼び出しの前にその記録を参照して計算を飛ばす最適化。ハッシュテーブル等を使う
* 末尾呼び出しは、最後に行う処理として自分自身や他の関数を呼び出すこと
  * 末尾呼び出し最適化は、末尾呼び出しにより余計なスタック操作を飛ばして効率化することを指す
* シャドウイングとは、外側と中側の関数で共通の変数名を使うときに中側の関数では近い方 (中側) の変数を優先する振舞
  * 混乱の元になるので嫌う人も居る
* ANSI Common Lisp 規格では末尾呼び出し最適化は要求されてない
  * Scheme では要求されている
  * 多くの Common Lisp コンパイラでは末尾呼び出し最適化をサポートしている (SBCL もそう)
