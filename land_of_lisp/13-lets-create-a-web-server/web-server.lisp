   ...
   (labels ((f (lst)
               (when lst
                 (case (car lst)
                     (#\% (cons (code-char
                                 (parse-integer
                                  (coerce (list (cadr lst) (caddr lst)) 'string)
                                  :radix 16
                                  :junk-allowed t))
                                (f (cdddr lst))))
...
; s is not URL, also request line. 
; arg (ex): "GET /lolcats.html HTTP/1.1"
;           "GET /lolcats.html?extra-funny=yes HTTP/1.1"
(defun parse-url (s) 
  (let* ((url (subseq s
                      (+ 2 (position #\space s))
...
(defun hello-request-handler (path header params)
  ...
  (if (equal path "greeting")
      ...
      (princ "<html>Sorry... I don't know that page.</html>")))